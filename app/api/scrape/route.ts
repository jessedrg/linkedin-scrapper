...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.PDL_API_KEY && !process.env.SERPER_API_KEY && !process.env.BRAVE_API_KEY)
    return NextResponse.json({ error: "No search provider configured (add PDL_API_KEY, SERPER_API_KEY, or BRAVE_API_KEY)" }, { status: 500 });
